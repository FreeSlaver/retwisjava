...
public class RedisService<K, V> {
	...
	public Set<V> zReverseRange(final K key,long start,long end){
		ZSetOperations<K, V> ops = redisTemplate.opsForZSet();
		return ops.reverseRange(key,start,end);
	}

	public Long zDelRange(final K key, long start, long end) {
		ZSetOperations<K, V> ops = redisTemplate.opsForZSet();
		return ops.removeRange(key, start, end);
	}
	public Long zDel(final  K key,Object ...values){
		ZSetOperations<K, V> ops = redisTemplate.opsForZSet();
